...
private:
	...
	//Function syntax: Func(std::string &in asTimer)
	...
	/**
	 * Callback syntax: MyFunc(std::string &in asName, int alCount), Count is 0 on first checkpoint load!
	 */
	...
	*/
	...
	/**
	* Callback syntax: MyFunc(std::string &in asItemA, std::string &in asItemB)
	*/
	...
	/**
	 * Callback syntax: MyFunc(std::string &in asItem, std::string &in asEntity)
	 */
	...
	* CrossHair can be: Default (uses default), Grab, Push, Ignite, Pick, LevelDoor, Ladder
	...
	/**
	* Callback syntax: MyFunc(std::string &in entity, int alState) state: 1=looking, -1=not looking
	*/
	static void __stdcall SetEntityPlayerLookAtCallback(std::string& asName, std::string& asCallback, bool abRemoveWhenLookedAt);
	/**
	* Callback syntax: MyFunc(std::string &in entity)
	*/
	static void __stdcall SetEntityPlayerInteractCallback(std::string& asName, std::string& asCallback, bool abRemoveOnInteraction);
	/**
	 * Callback syntax: MyFunc(std::string &in entity, std::string &in type). Type depends on entity type and includes: "OnPickup", "Break", "OnIgnite", etc
	 */
	static void __stdcall SetEntityCallbackFunc(std::string& asName, std::string& asCallback);
	/**
	* A callback called when ever the connection state changes (button being switched on). Syntax: void Func(std::string &in EntityName, int alState). alState: -1=off, 0=between 1=on
	*/
	...
	*/
	...
	/**
	 * Called when state changes Syntax: MyFunc(std::string &in asEntity, int alState)
	 */
	...
	/**
	 * Callback syntax: MyFunc(std::string &in asProp)
	 */
	...
	*/
	...
	// Parent can have asterix in name but mot child! Entity callbacks alStates = 1=only enter, -1=only leave 0=both. Syntax: void Func(std::string &in asParent, std::string &in asChild, int alState). alState: 1=enter, -1=leave.
	...
	static void __stdcall InteractConnectPropWithMoveObject(std::string& asName, std::string& asPropName, std::string& asMoveObjectName, bool abInteractOnly,
															bool abInvert, int alStatesUsed);
	/**
	 * Callback Syntax: MyFunc(std::string &in asConnectionName, std::string &in asMainEntity, std::string &in asConnectEntity, int alState). State is what is sent to connection entity and will be inverted if abInvertStateSent=true!
	 */
