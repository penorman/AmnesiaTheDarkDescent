...
void RegisterScriptString_Native(asIScriptEngine *engine)
{
	...
	if( sizeof(size_t) == 4 )
	{
		...
		r = engine->RegisterObjectMethod("string", "void resize(uint)", asSMethodPtr<sizeof(void (string::*)())>::Convert(reinterpret_cast<void (string::*)(size_t)>(&string::resize)), asCALL_THISCALL); assert(r >= 0);
	}
	else
	{
		r = engine->RegisterObjectMethod("string", "uint64 length() const", asMETHOD(string,size), asCALL_THISCALL); assert( r >= 0 );
		r = engine->RegisterObjectMethod("string", "void resize(uint64)", asSMethodPtr<sizeof(void (string::*)())>::Convert(reinterpret_cast<void (string::*)(size_t) >(&string::resize)), asCALL_THISCALL); assert(r >= 0);
	}
